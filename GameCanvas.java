...
public class GameCanvas extends Canvas implements Runnable {
    ...
    private static final int S_OYUN=3,S_FLASH=4,S_TUR_BIT=5,S_KAZANAN=6,S_ISTAT=7,S_TAKIM_AD=8,S_KATEGORI=9,S_DEBUG=10;
    ...
    private static final int[][] TM = {
        ...
        {0xE8F4FD,0xD0E8F8,0xB0D8F0,0x88C4E8,0x2196F3,0x00E5FF,0xFFD700,0x001830,0x4488BB,0x1A3A5C,0xADD8E6,0x87CEEB},
        {0xFF6B35,0xFF8C42,0xFFA552,0xFFBD69,0xFF4500,0xFFD700,0xFF6B35,0xFFF8F0,0xCC5500,0xAA3300,0xFFAA44,0xFF6600},
        {0x001F3F,0x003366,0x004080,0x0055AA,0x0088FF,0x00FFCC,0xFFD700,0xE0F4FF,0x3377BB,0x001833,0x66BBFF,0x00AAFF},
        {0x1A0A00,0x2D1400,0x401E00,0x5C2A00,0xE8A020,0xFF6600,0xFFCC00,0xFFF5E0,0xBB7700,0x663300,0xFFCC44,0xFF8800},
        {0xF0FFF0,0xE0FFE0,0xC8FFC8,0xAAFFAA,0x00CC44,0x00FFAA,0xFFEE00,0x001A00,0x44BB44,0x003300,0x88FF88,0x00FF66},
        {0x0A0A2E,0x16213E,0x0F3460,0x533483,0xE94560,0xF5A623,0xFFD700,0xEEEEFF,0x7755AA,0x221133,0xBB88FF,0xFF55AA},
        {0xFFF5F0,0xFFE8DC,0xFFD4C0,0xFFC0A0,0xFF6B35,0xFF9500,0xFF3366,0x2D0A00,0xBB6644,0x661A00,0xFF9966,0xFF6633},
        {0x001A33,0x003366,0x004499,0x0055BB,0xC8102E,0xFFFFFF,0x002868,0xFFFFFF,0x0044AA,0x001133,0xEEEEFF,0xCC1122}
    };
    private static final String[][] TM_AD={
        {"Koyu","Dark"},{"Pembe K.","Pink D."},{"Pembe A.","Pink L."},
        {"Mavi","Blue"},{"Turuncu","Orange"},{"Orman","Forest"},{"Mor","Purple"},{"Sari","Yellow"},{"Buz","Ice"},{"Alev","Flame"},{"Okyanus","Ocean"},{"Altin","Gold"},{"Neon","Neon"},{"Galaksi","Galaxy"},{"Gunbatimi","Sunset"},{"Turkiye","Turkey"}
    };
    ...
    // Emoji resim cache - path -> Image
    private String[] eCachePath = new String[30];
    private Image[]  eCacheImg  = new Image[30];
    private int      eCacheSize = 0;

    private Image getEmojiImg(String path) {
        if (path==null) return null;
        for (int i=0;i<eCacheSize;i++) if (eCachePath[i].equals(path)) return eCacheImg[i];
        try {
            InputStream is=getClass().getResourceAsStream("/"+path);
            if (is==null) return null;
            Image img=Image.createImage(is);
            if (eCacheSize<30) { eCachePath[eCacheSize]=path; eCacheImg[eCacheSize]=img; eCacheSize++; }
            return img;
    ...
    // Splash emoji - menu animasyonu
    private String splashEmojiPath=null;
    private long   splashStart2=0;
    private static final String[] SPLASH_EMOJIS={
        "e/sport_0.png","e/sport_3.png","e/sport_7.png","e/sport_16.png","e/sport_24.png",
        "e/sky_weather_13.png","e/sky_weather_8.png","e/sky_weather_34.png","e/sky_weather_40.png",
        "e/heart_11.png","e/heart_14.png","e/hands_0.png","e/hands_6.png","e/hands_2.png",
        "e/plant_flower_5.png","e/plant_flower_8.png","e/food_prepared_13.png","e/food_prepared_15.png",
        "e/hand_fingers_closed_0.png","e/hand_fingers_partial_3.png","e/face_unwell_0.png",
        "e/geometric_0.png","e/geometric_3.png","e/sky_weather_44.png","e/sky_weather_46.png"
    };
    ...
    private int menuSec=0,kategoriSec=0;
    private int konamiIdx=0,dbSec=0;
    private static final int[] KONAMI={UP,UP,DOWN,DOWN,LEFT,RIGHT,LEFT,RIGHT};
    private boolean debugMod=false;
    private int adTakim=0,adSec=0;

    // Kategori secimi
    // 0=Mix(eski), 1=Kis, 2=Bahar, 3=Tatil, 4=Gece Yarisi, 5=Unluler, 6=Genel Kultur
    private static final String[] KAT_TR={"Mix (Klasik)","Kis","Bahar","Tatil","Gece Yarisi","Unluler","Genel Kultur"};
    private static final String[] KAT_EN={"Mix (Classic)","Winter","Spring","Vacation","Midnight","Celebrities","General"};
    // Kategori baslangic indexleri TabuData icinde (yakla\u015f\u0131k)
    // Mix = 0..1751 (eski kartlar), yeniler 1752'den sonra
    private static final int KAT_BITIS_KLASIK = 1751; // Eski kartlar\u0131n sonu
    ...
    public GameCanvas(TabuMIDlet m) {
        ...
        T=TM[temaIdx];
        resimYukle();
        // Rastgele splash emoji sec
        splashEmojiPath=SPLASH_EMOJIS[(int)(Math.random()*SPLASH_EMOJIS.length)];
        splashStart2=System.currentTimeMillis();
        muzikBaslat();
    ...
    protected void paint(Graphics g){
        ...
        switch(ekran){
            ...
            case S_KATEGORI: pKategori(g); break;
            case S_DEBUG:    pDebug(g);    break;
    ...
    private void pMenu(Graphics g){
        ...
        // Ust alan - T harfi (biraz daha kucuk - H/4)
        int headerH=H/4;
        fR(g,T[2],0,0,W,headerH);
        int px=W/2-24,py=6,ps=7;
        g.setColor(T[10]);g.fillRect(px,py,48,ps*2);g.fillRect(px+17,py+ps*2,14,ps*4);
        g.setColor(drk(T[10]));g.fillRect(px+2,py+2,46,ps*2-2);g.fillRect(px+19,py+ps*2+2,12,ps*4-2);
        g.setColor(lgt(T[10]));g.fillRect(px+4,py+2,8,3);
        int lh=headerH;

        // TABU - renk animasyonu
        int tc=blnd(T[10],T[11],(int)(128+127*Math.sin(now/500.0)));
        g.setColor(tc);g.setFont(fL);ciz(g,dil==0?"TABU":"TABOO",W/2,lh+1);

        // Bayrak + dil etiketi
        int flagY=lh+3;
        Image flagImg=dil==0?imgTR:imgEN;
        if(flagImg!=null){
            g.drawImage(flagImg,W-flagImg.getWidth()-5,flagY,Graphics.TOP|Graphics.LEFT);
        } else {
        ...
        // 10.000+ KELIME - saka icin, zipla
        int by=(int)(3*Math.sin(now/300.0));
        int br=(int)(200+55*Math.sin(now/400.0));
        g.setColor((br<<16)|((br*200/255)<<8)|0);
        g.setFont(fSB);
        String kelimeTx=dil==0?"10.000+ KELIME":"10,000+ WORDS";
        ciz(g,kelimeTx,W/2,lh+fL.getHeight()+4+by);

        // Splash emoji - piksel buyutme ile daha buyuk (hedef ~48-64px)
        if(splashEmojiPath!=null){
            Image sem=getEmojiImg(splashEmojiPath);
            if(sem!=null){
                long elapsed2=now-splashStart2;
                if(elapsed2>2500){
                    Random rndEm=new Random(now);
                    splashEmojiPath=SPLASH_EMOJIS[(int)(Math.abs(rndEm.nextInt())%SPLASH_EMOJIS.length)];
                    splashStart2=now; elapsed2=0;
                }
                double phase=elapsed2/2500.0;
                double scale;
                if(phase<0.4)      scale=3.0+9.0*(phase/0.4);  // 3x -> 12x
                else if(phase<0.6) scale=12.0;                  // dur
                else               scale=12.0-9.0*((phase-0.6)/0.4); // 12x -> 3x
                if(scale<3.0)scale=3.0;
                int sw=sem.getWidth(),sh=sem.getHeight();
                int psz=(int)scale; if(psz<1)psz=1;
                int ds=sw*psz, dsh=sh*psz;
                int ex=W/2-ds/2, ey=py+ps-dsh/2;
                try{
                    int[] rgb=new int[sw*sh];
                    sem.getRGB(rgb,0,sw,0,0,sw,sh);
                    for(int row=0;row<sh;row++){
                        for(int col=0;col<sw;col++){
                            int c2=rgb[row*sw+col];
                            if((c2>>>24)>64){
                                g.setColor(c2&0xFFFFFF);
                                g.fillRect(ex+col*psz,ey+row*psz,psz,psz);
                            }
                        }
                    }
                }catch(Exception ex2){
                    g.drawImage(sem,W/2,py+ps,Graphics.VCENTER|Graphics.HCENTER);
                }
            }
        }
        ...
        if(enSkor>0){
            ...
            ciz(g,cat4("Rekor: ",iv(enSkor)," - ",enTakim),W/2,lh+fL.getHeight()+fSB.getHeight()+10);
        }

        String[] items=dil==0
            ?new String[]{"OYUNA BASLA","AYARLAR","TAKIM ADLARI","ISTATISTIK","CIKIS"}
            :new String[]{"START GAME","SETTINGS","TEAM NAMES","STATISTICS","EXIT"};
        int btnH=Math.max(22,H/12),btnW=W*4/5,bX=(W-btnW)/2;
        int sY=lh+fL.getHeight()+fSB.getHeight()+(enSkor>0?fSB.getHeight()+4:0)+2;
    ...
    private void pOyun(Graphics g){
        ...
        // Emoji goster - 32x32 piksel buyutmeli
        String ePath=dil==0?EmojiData.getImg(idx):EmojiDataEN.getImg(idx);
        if (ePath!=null) {
            Image em=getEmojiImg(ePath);
            if (em!=null) {
                int sw=em.getWidth(),sh=em.getHeight();
                int tgt=32; // hedef boyut
                int psz=tgt/sw; if(psz<1)psz=1;
                int ds=sw*psz, dsh=sh*psz;
                int ex=W/2+kartOff+fL.stringWidth(ana)/2+6;
                int ey=kY+8;
                int[] rgb=new int[sw*sh];
                em.getRGB(rgb,0,sw,0,0,sw,sh);
                for(int row=0;row<sh;row++){
                    for(int col=0;col<sw;col++){
                        int c2=rgb[row*sw+col];
                        if((c2>>>24)>64){
    ...
    protected void keyPressed(int k){
        ...
        switch(ekran){
            ...
            case S_KATEGORI: katTus(k,a);break;
    ...
    private void konamiKontrol(int a){
        if(KONAMI[konamiIdx]==a){konamiIdx++;if(konamiIdx==KONAMI.length){debugMod=!debugMod;konamiIdx=0;}}
        else konamiIdx=0;
    }
    ...
    private void mTus(int k,int a){
        ...
        if(a==FIRE||k==K5){
            switch(menuSec){
                case 0:ekran=S_KATEGORI;break; // Once kategori sec
    ...
    private void katTus(int k,int a){
        String[] kats=dil==0?KAT_TR:KAT_EN;
        if(k==KSTAR){ekran=S_MENU;return;}
        if(a==UP||k==K2)kategoriSec=(kategoriSec-1+kats.length)%kats.length;
        if(a==DOWN||k==K8)kategoriSec=(kategoriSec+1)%kats.length;
        if(a==FIRE||k==K5)oyunBaslat();
        if(debugMod&&(k==KHASH)){ekran=S_DEBUG;dbSec=0;}
    }
    ...
    // \u2500\u2500 Takim Adi Listesi \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
    private void pKategori(Graphics g){
        Font fM=fl(Font.SIZE_MEDIUM),fS=fp(Font.SIZE_SMALL),fSB=fb(Font.SIZE_SMALL);
        fR(g,T[2],0,0,W,32);
        g.setColor(T[10]);g.setFont(fM);
        ciz(g,dil==0?"KART KATEGORISI":"CARD CATEGORY",W/2,8);
        g.setColor(T[8]);g.setFont(fS);
        g.drawString(dil==0?"* Geri":"* Back",5,10,Graphics.TOP|Graphics.LEFT);

        String[] kats=dil==0?KAT_TR:KAT_EN;
        int rH=Math.max(28,(H-40)/kats.length-3);
        int rW=W-20,rX=10;
        for(int i=0;i<kats.length;i++){
            int rY=38+i*(rH+4);
            boolean sel=(i==kategoriSec);
            fRnd(g,sel?T[4]:T[1],rX,rY,rW,rH);
            if(sel){g.setColor(T[7]);g.drawRoundRect(rX,rY,rW,rH,8,8);}
            g.setColor(sel?T[7]:T[8]);g.setFont(fM);
            ciz(g,kats[i],W/2,rY+rH/2-fM.getHeight()/2);
        }
        g.setColor(T[9]);g.setFont(fS);
        ciz(g,dil==0?"2/8:Sec  5:Baslat":"2/8:Select  5:Start",W/2,H-fS.getHeight()-2);
    }
...
private void pTakimAd(Graphics g){
    ...
    private void oyunBaslat(){
        ...
        if(kategoriSec==0){
            int maks=dil==0?TabuData.KLASIK_BITIS:EngData.KLASIK_BITIS;
            deste=karistir(maks);
        }else{
            int bas,bit;
            if(dil==0){
                int[][] r={{TabuData.KIS_BAS,TabuData.KIS_BIT},{TabuData.BAHAR_BAS,TabuData.BAHAR_BIT},{TabuData.TATIL_BAS,TabuData.TATIL_BIT},{TabuData.GECEYARISI_BAS,TabuData.GECEYARISI_BIT},{TabuData.UNLULER_BAS,TabuData.UNLULER_BIT},{TabuData.GENELKULTUR_BAS,TabuData.GENELKULTUR_BIT}};
                bas=r[kategoriSec-1][0];bit=r[kategoriSec-1][1];
            }else{
                int[][] r={{EngData.WINTER_BAS,EngData.WINTER_BIT},{EngData.SPRING_BAS,EngData.SPRING_BIT},{EngData.HOLIDAY_BAS,EngData.HOLIDAY_BIT},{EngData.MIDNIGHT_BAS,EngData.MIDNIGHT_BIT},{EngData.CELEBRITIES_BAS,EngData.CELEBRITIES_BIT},{EngData.GENERALKNOWLEDGE_BAS,EngData.GENERALKNOWLEDGE_BIT}};
                bas=r[kategoriSec-1][0];bit=r[kategoriSec-1][1];
            }
            deste=karistirAralik(bas,bit);
        }
        dIdx=0;ekran=S_HAZIR;
    }

    private int[] karistirAralik(int bas,int bit){
        int n=bit-bas; if(n<=0)return karistir(TabuData.TOPLAM);
        int[] a=new int[n]; for(int i=0;i<n;i++)a[i]=bas+i;
        Random r=new Random();
        for(int i=n-1;i>0;i--){int j=(r.nextInt()&0x7FFFFFFF)%(i+1);int t=a[i];a[i]=a[j];a[j]=t;}
        return a;
    }

    private int[] themeRange(int idx){
        // Yeni kategoriler 800'den sonra, her biri ~120 kart
        int base=800;
        switch(idx){
            case 0: return new int[]{base,       base+120}; // Kis
            case 1: return new int[]{base+120,   base+240}; // Bahar
            case 2: return new int[]{base+240,   base+360}; // Tatil
            case 3: return new int[]{base+360,   base+480}; // Gece Yarisi
            case 4: return new int[]{base+480,   base+600}; // Unluler
            case 5: return new int[]{base+600,   TabuData.TOPLAM}; // Genel Kultur
            default: return new int[]{0,          800}; // Mix klasik
        }
    }
    ...
    private void pDebug(Graphics g){
        Font fM=fl(Font.SIZE_MEDIUM),fS=fp(Font.SIZE_SMALL);
        fR(g,0x001100,0,0,W,H);
        g.setColor(0x00FF44);g.setFont(fM);
        ciz(g,"** DEBUG **",W/2,4);
        // Emoji listesi - 8 satirlik grid
        int cols=W/20,rows=(H-30)/20;
        int start=dbSec*cols*rows;
        for(int i=0;i<cols*rows;i++){
            int idx2=start+i;
            if(idx2>=EmojiData.PATHS.length)break;
            int gx=(i%cols)*20+2,gy=30+(i/cols)*20;
            try{
                Image em=getEmojiImg(EmojiData.PATHS[idx2]);
                if(em!=null){
                    int sw=em.getWidth(),sh=em.getHeight();
                    int psz=Math.max(1,16/sw);
                    int[] rgb=new int[sw*sh];
                    em.getRGB(rgb,0,sw,0,0,sw,sh);
                    for(int rr=0;rr<sh;rr++) for(int cc=0;cc<sw;cc++){
                        int col2=rgb[rr*sw+cc];
                        if((col2>>>24)>64){g.setColor(col2&0xFFFFFF);g.fillRect(gx+cc*psz,gy+rr*psz,psz,psz);}
                    }
                }
            }catch(Exception ignored){}
        }
        g.setColor(0x00FF44);g.setFont(fS);
        ciz(g,"2/8:Sayfa  *:Cik  Sayfa:"+iv(dbSec+1),W/2,H-fS.getHeight()-2);
    }

    private void dbTus(int k,int a){
        if(k==KSTAR){ekran=S_MENU;return;}
        int maxPages=(EmojiData.PATHS.length/(W/20*((H-30)/20)))+1;
        if(a==DOWN||k==K8)dbSec=(dbSec+1)%maxPages;
        if(a==UP||k==K2)dbSec=(dbSec-1+maxPages)%maxPages;
    }
